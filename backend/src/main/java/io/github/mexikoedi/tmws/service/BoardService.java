/**
 * Diese Klasse implementiert die Geschäftslogik für die Verwaltung von Projektboards, Statuskategorien und Aufgaben.
 */
...
public class BoardService {
  ...
  /**
   * Gibt eine Liste aller Projektboards zurück, auf die der Benutzer Zugriff hat, einschließlich aller zugehörigen
   * Statuskategorien und Aufgaben.
   *
   * @param email Die E-Mail-Adresse des Benutzers, für den die Projektboards abgerufen werden sollen.
   * @throws ResourceNotFoundException Wenn kein Benutzer mit der angegebenen E-Mail-Adresse gefunden wird.
   * @return Eine Liste von BoardResponse-Objekten, die die Projektboards und ihre Details repräsentieren.
   */
  public List<BoardResponse> listBoards(String email) {
    ...
    return boardRepository.findBoardsForUserWithRelations(user).stream().map(this::mapToResponse).collect(Collectors.toList());
  }

  /**
   * Gibt ein bestimmtes Projektboard zurück, einschließlich aller zugehörigen Statuskategorien und Aufgaben.
   *
   * @param id Die ID des Projektboards, das abgerufen werden soll.
   * @return Ein Optional, das das Board-Objekt enthält, wenn es gefunden wurde, oder leer ist, wenn kein Board
   * mit der angegebenen ID existiert.
   */
  ...
  /**
   * Erstellt ein neues Projektboard mit den angegebenen Details und setzt den angegebenen Benutzer als Besitzer
   * und erstes Mitglied.
   *
   * @param board Die Details des zu erstellenden Projektboards, einschließlich Titel und optionalem Hintergrund.
   * @param ownerEmail Die E-Mail-Adresse des Benutzers, der als Besitzer des Projektboards festgelegt werden soll.
   * @throws ResourceNotFoundException Wenn kein Benutzer mit der angegebenen E-Mail-Adresse gefunden wird.
   * @return Das erstellte Board-Objekt mit allen zugehörigen Details, einschließlich der automatisch erstellten
   * Statuskategorien.
   */
  @Transactional
  public Board createBoard(Board board, String ownerEmail) {
    User owner = userRepository.findByEmail(ownerEmail).orElseThrow(() -> new ResourceNotFoundException("Benutzer existiert nicht."));
   ...
   * @param request Ein Objekt, das die neuen Details des Projektboards enthält, einschließlich Titel und Hintergrund.
  ...
  public Board updateBoard(Long boardId, UpdateBoardRequest request) {
    Board board = boardRepository.findByIdWithRelations(boardId).orElseThrow(() -> new ResourceNotFoundException("Projektboard nicht gefunden."));
    board.setTitle(request.getTitle());
    board.setBackground((request.getBackground() == null || request.getBackground().isBlank()) ? null : request.getBackground());
    Board saved = boardRepository.save(board);
    websocket.notifyBoardAndMembers(saved);
  ...
  /**
   * Lädt einen Benutzer ein, Mitglied eines bestehenden Projektboards zu werden, basierend auf der E-Mail-Adresse
   * des Benutzers.
   *
   * @param boardId Die ID des Projektboards, zu dem der Benutzer eingeladen werden soll.
   * @param request Ein Objekt, das die E-Mail-Adresse des Benutzers enthält, der eingeladen werden soll.
   * @throws ResourceNotFoundException Wenn kein Projektboard mit der angegebenen ID gefunden wird oder wenn kein
   * Benutzer mit der angegebenen E-Mail-Adresse gefunden wird.
   * @throws UserDeactivatedException Wenn der Benutzer, der eingeladen werden soll, deaktiviert ist.
   * @throws UserInviteNotPossibleException Wenn versucht wird, den Besitzer des Projektboards einzuladen.
  ...
  public Board invite(Long boardId, InviteBoardRequest request) {
    Board board = boardRepository.findByIdWithRelations(boardId).orElseThrow(() -> new ResourceNotFoundException("Projektboard nicht gefunden."));
    User user = userRepository.findByEmail(request.getEmail()).orElseThrow(() -> new ResourceNotFoundException("Benutzer existiert nicht."));
  ...
  /**
   * Fügt eine neue Statuskategorie zu einem bestehenden Projektboard hinzu, basierend auf dem Titel der Kategorie.
   *
   ...
   * @throws ResourceNotFoundException Wenn kein Projektboard mit der angegebenen ID gefunden wird.
   * @throws ColumnAlreadyExistsException Wenn bereits eine Statuskategorie mit demselben Titel im Projektboard existiert.
   * @return Ein BoardColumnResponse-Objekt, das die Details der neu hinzugefügten Statuskategorie enthält.
   */
  @Transactional
  public BoardColumnResponse addColumn(Long boardId, UpdateBoardColumnRequest request) {
    Board board = boardRepository.findByIdWithRelations(boardId).orElseThrow(() -> new ResourceNotFoundException("Projektboard nicht gefunden."));
    boolean exists = board.getColumns().stream().anyMatch(c -> c.getTitle().equalsIgnoreCase(request.getTitle()));
  ...
  /**
   * Aktualisiert die Details einer bestehenden Statuskategorie, einschließlich des Titels, basierend auf der ID
   * der Kategorie.
   *
   * @param columnId Die ID der Statuskategorie, die aktualisiert werden soll.
   * @param request Ein Objekt, das die neuen Details der Statuskategorie enthält, einschließlich des Titels.
   * @throws ResourceNotFoundException Wenn keine Statuskategorie mit der angegebenen ID gefunden wird.
   * @throws ColumnAlreadyExistsException Wenn bereits eine andere Statuskategorie mit demselben Titel im
  ...
  public Board updateColumn(Long columnId, UpdateBoardColumnRequest request) {
    BoardColumn col = columnRepository.findByIdWithRelations(columnId).orElseThrow(() -> new ResourceNotFoundException("Statuskategorie nicht gefunden."));
    Board board = col.getBoard();

    if (request.getTitle() != null && !request.getTitle().isBlank()) {
      boolean exists = board.getColumns().stream().anyMatch(c -> c.getTitle().equalsIgnoreCase(request.getTitle()) && !c.getId().equals(columnId));
  ...
  /**
   * Verschiebt eine Statuskategorie innerhalb eines Projektboards an eine neue Position, basierend auf der ID
   * der Kategorie und der neuen Position.
   *
   * @param columnId Die ID der Statuskategorie, die verschoben werden soll.
   * @param position Die neue Position, an die die Statuskategorie verschoben werden soll (0-basiert).
   * @throws ResourceNotFoundException Wenn keine Statuskategorie mit der angegebenen ID gefunden wird.
   */
  @Transactional
  public void moveColumn(Long columnId, int position) {
    BoardColumn column = columnRepository.findByIdWithRelations(columnId).orElseThrow(() -> new ResourceNotFoundException("Statuskategorie nicht gefunden."));
  ...
  /**
   * Fügt eine neue Aufgabe zu einer bestehenden Statuskategorie hinzu, basierend auf der ID der Kategorie und den
   * Details der Aufgabe.
   *
   ...
   * @throws ResourceNotFoundException Wenn keine Statuskategorie mit der angegebenen ID gefunden wird.
   * @throws TaskAlreadyExistsException Wenn bereits eine Aufgabe mit demselben Titel in der Statuskategorie existiert.
  ...
  public TaskResponse  addTask(Long columnId, UpdateTaskRequest request) {
    BoardColumn column = columnRepository.findByIdWithRelations(columnId).orElseThrow(() -> new ResourceNotFoundException("Statuskategorie nicht gefunden."));
    Board board = column.getBoard();
    boolean exists = board.getColumns().stream().flatMap(c -> c.getTasks().stream()).anyMatch(t -> t.getTitle().equalsIgnoreCase(request.getTitle()));
  ...
  /**
   * Aktualisiert die Details einer bestehenden Aufgabe, basierend auf der ID der Aufgabe und den neuen Details.
   *
  ...
  public Task updateTask(Long taskId, UpdateTaskRequest request) {
    Task task = taskRepository.findByIdWithRelations(taskId).orElseThrow(() -> new ResourceNotFoundException("Aufgabe nicht gefunden."));
    Board board = task.getColumn().getBoard();
    boolean exists = board.getColumns().stream().flatMap(c -> c.getTasks().stream()).anyMatch(t -> t.getTitle().equalsIgnoreCase(request.getTitle()) && !t.getId().equals(taskId));
   ...
   * basierend auf der ID der Aufgabe, der ID der Zielkategorie und der neuen Position innerhalb der Zielkategorie.
   *
   * @param taskId Die ID der Aufgabe, die verschoben werden soll.
   * @param targetColumnId Die ID der Ziel-Statuskategorie, zu der die Aufgabe verschoben werden soll.
   * @param position Die neue Position, an die die Aufgabe innerhalb der Ziel-Statuskategorie verschoben werden
   * soll (0-basiert).
   * @throws ResourceNotFoundException Wenn keine Aufgabe mit der angegebenen ID gefunden wird oder wenn keine
   * Statuskategorie mit der angegebenen Ziel-ID gefunden wird.
   * @return Ein TaskResponse-Objekt, das die Details der verschobenen Aufgabe enthält, einschließlich der neuen
   * Position und der neuen Statuskategorie.
   */
  @Transactional
  public TaskResponse moveTask(Long taskId, Long targetColumnId, int position) {
    Task task = taskRepository.findTaskWithColumn(taskId).orElseThrow(() -> new ResourceNotFoundException("Aufgabe nicht gefunden."));
    BoardColumn oldColumn = task.getColumn();
    BoardColumn targetColumn = columnRepository.findColumnWithTasks(targetColumnId).orElseThrow(() -> new ResourceNotFoundException("Ziel-Statuskategorie nicht gefunden."));
    ...
    List<Task> tasks = targetColumn.getTasks().stream().filter(t -> !Objects.equals(t.getId(), taskId)).sorted(Comparator.comparingInt(Task::getPosition)).toList();
   ...
   * @return Ein BoardResponse-Objekt, das die Details des Projektboards und seiner zugehörigen Statuskategorien,
   * Aufgaben und Mitglieder enthält.
   */
  public BoardResponse mapToResponse(Board board) {
    ...
    response.setColumns(board.getColumns().stream().sorted(Comparator.comparingInt(BoardColumn::getPosition)).map(c -> getBoardColumnResponse(board, c)).toList());
    ...
    response.setMembers(allMembers.stream().filter(User::isEnabled).sorted(Comparator.comparingLong(User::getId)).map(UserResponse::new).toList());
   ...
   *
   * @param board Das Board-Objekt, zu dem die Statuskategorie gehört, um die BoardId im Response setzen zu können.
   * @param saved Das BoardColumn-Objekt, das in ein BoardColumnResponse-Objekt konvertiert werden soll.
   * @return Ein BoardColumnResponse-Objekt, das die Details der Statuskategorie und ihrer zugehörigen Aufgaben enthält.
   */
  @NonNull
  private BoardColumnResponse getBoardColumnResponse(Board board, BoardColumn saved) {
    ...
    response.setTasks(saved.getTasks().stream().sorted(Comparator.comparingInt(Task::getPosition)).map(TaskResponse::fromEntity).collect(Collectors.toList()));

    return response;
  }

  /**
   * Hilfsmethode, um eine neue Statuskategorie mit den angegebenen Details zu erstellen und sie einem
   * Projektboard zuzuordnen.
   *
   ...
   * @return Ein BoardColumn-Objekt, das die Details der neu erstellten Statuskategorie enthält und mit dem
   * Projektboard verknüpft ist.
   */
   ...
   *
   * @param task Die Aufgabe, deren Zuweisung sich geändert hat, um die Details für die E-Mail zu verwenden.
   * @param previous Die vorherige Menge von Benutzern, die der Aufgabe zugewiesen waren, um zu bestimmen, welche
   * Benutzer eine E-Mail über die Entfernung erhalten sollen.
   * @param current Die aktuelle Menge von Benutzern, die der Aufgabe zugewiesen sind, um zu bestimmen,
   * welche Benutzer eine E-Mail über die neue Zuweisung erhalten sollen.
   */
